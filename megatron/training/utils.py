...
def throughput_calculator(args, iteration_time, total_iterations):
    ...
    #             assert args.recompute_num_layers_of_stages is not None
    #             recompute_layers = 0
    #             for i in range(len(args.recompute_num_layers_of_stages)):
    #                 recompute_layers += args.recompute_num_layers_of_stages[i]
    #             checkpoint_activations_factor = 3 + 1. * (recompute_layers / args.num_layers)
    #     else :
    ...
    flops_per_iteration = (24 * checkpoint_activations_factor * batch_size * seq_len * num_layers * (hidden_size**2)) * (1. + (seq_len / (6. * selective_recompute_factor * hidden_size)))+ (6 * batch_size * seq_len *hidden_size * vocab_size)
    tflops = flops_per_iteration / (elapsed_time_per_iter * args.world_size * (10**12))

    # tgs=(tokens * global_batchsize) / steptime / gpu_number
    tgs = samples_per_model / iteration_time / args.world_size
    return samples_per_second, tflops, tgs